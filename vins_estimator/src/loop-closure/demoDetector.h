...
template <class TVocabulary, class TDetector, class TDescriptor>
demoDetector<TVocabulary, TDetector, TDescriptor>::demoDetector(const std::string &vocfile, int width, int height) :
...
template <class TVocabulary, class TDetector, class TDescriptor>
void demoDetector<TVocabulary, TDetector, TDescriptor>::initCameraModel(const std::string &calib_file) {
    detector.initCameraModel(calib_file);
}

// ---------------------------------------------------------------------------
template <class TVocabulary, class TDetector, class TDescriptor>
void demoDetector<TVocabulary, TDetector, TDescriptor>::eraseIndex(std::vector<int> &erase_index) {
...
template <class TVocabulary, class TDetector, class TDescriptor>
bool demoDetector<TVocabulary, TDetector, TDescriptor>::run(const std::string &name, const std::vector<cv::KeyPoint> &keys,
                                                            const std::vector<TDescriptor> &descriptors,
                                                            std::vector<cv::Point2f> &cur_pts,
                                                            std::vector<cv::Point2f> &old_pts,
                                                            int &old_index) {
...
template <class TVocabulary, class TDetector, class TDescriptor>
void demoDetector<TVocabulary, TDetector, TDescriptor>::readPoseFile(const char *filename, std::vector<double> &xs, std::vector<double> &ys)
    const {
    xs.clear();
    ys.clear();
